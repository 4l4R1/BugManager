...
Public Class TargaFile
    ...
    Public Shared Sub SaveAsTarga(Filename As String, Picture As Bitmap)
        ...
        Dim premultpict As Bitmap = Picture.Clone(New Rectangle(0, 0, Picture.Width, Picture.Height), Imaging.PixelFormat.Format32bppPArgb)
        'Picture.Dispose()
        'System.Drawing.Bitmap have there pixels arranged from top to bottom, TGA's from bottom to top, so we flip the picture
        premultpict.RotateFlip(RotateFlipType.RotateNoneFlipY)
        ...
        Using FS As New FileStream(Filename, FileMode.Create, FileAccess.Write)
            ...
            Using bw As BinaryWriter = New BinaryWriter(FS)
                ...
                sh = CShort(premultpict.Width)
                bw.Write(sh)         'Width
                sh = CShort(premultpict.Height)
                bw.Write(sh)         'Height
                ...
                Dim bmpData As BitmapData = premultpict.LockBits(New Rectangle(0, 0, premultpict.Width, premultpict.Height), ImageLockMode.ReadOnly, PixelFormat.Format32bppPArgb)

                Dim ptr As IntPtr = bmpData.Scan0
                Dim bytes As Integer = bmpData.Stride * premultpict.Height
                Dim rgbValues(bytes - 1) As Byte

                System.Runtime.InteropServices.Marshal.Copy(ptr, rgbValues, 0, bytes)
                bw.Write(rgbValues)

                premultpict.UnlockBits(bmpData)
